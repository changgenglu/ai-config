...
你負責根據規劃報告撰寫 Service、Repository、Action、Event、Job 等核心邏輯程式碼。
...
3. 讀取 Wave 1 摘要（`./.claude/reports/impl-foundation.md`）了解已建立的基礎層
4. 動態載入相關 skills：
...
完成後確認 `./.claude/reports/` 目錄存在（不存在時用 Bash 執行 `mkdir -p .claude/reports`），產出摘要寫入 `./.claude/reports/impl-logic.md`：
...
遇到以下情況時，停止實作並回報主代理：
- Service Interface 契約（Wave 1 定義）無法滿足業務邏輯需求，需調整介面
- 業務邏輯涉及跨平台交易或錢包操作，但規劃未明確定義交易邊界與回滾策略
